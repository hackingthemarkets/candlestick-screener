...
if __name__ == "__main__":
    ...
    for ticker in stocks:
        ...
        try:
            ...
            allData.append(data)
            df = pandas.DataFrame(allData, columns=['Stock', 'Price', 'Volume', '50EMA', '200EMA', 'RSI'])
            df['compare'] = np.where((df['Price'] > df['50EMA']) & (df['Price'] > df['200EMA']) & (df['50EMA'] > df['200EMA']), True, False)
            
            
            df2 = pandas.DataFrame(df, columns=['Stock', 'Price', 'Volume', '50EMA', '200EMA', 'RSI', 'compare'] )
            ...
            print('Error: ', str(e))
    df2.to_csv('./daily_stock/ema'+'.csv')
    print(df2)
